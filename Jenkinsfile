...
pipeline {
	...
	parameters {
		...
		buildSelector defaultSelector: specific('env.BUILD_NUMBER'), name: 'BUILDSELECTOR', description: 'Image to perform integration tests on. Changing the default value skips the image build.'
	}
	...
	//			}
					...
					stage ('Build image') {
						steps {
								...
								if (env.CHANGE_TARGET == null && env.PR_BRANCHES == "" && env.YOCTO_VERSION == env.BRANCH_NAME) {
									stepBuildImage(workspace: "${WORKSPACE}", gyroid_arch: "x86", gyroid_machine: "genericx86-64", buildtype: "${BUILDTYPE}", build_installer: "n", sync_mirrors: "y")
								} else {
				...
				stages {
					stage ('Perform tests') {
						steps {
							...
							sh "ls /yocto_mirror"
							stepIntegrationTest(workspace: "${WORKSPACE}", buildtype: "${BUILDTYPE}", buildselector: params.BUILDSELECTOR, schsm_serial: "", schsm_pin: "")
						}
					} // stage 'Perform tests'
				} // stages
			} // matrix
		...
		stage ('Token Tests') {
			...
			steps {
				sh "echo ${env.BUILDNODE}"
				sh "ls /yocto_mirror"
				stepIntegrationTest(workspace: "${WORKSPACE}", buildSelector: params.BUILDSELECTOR, buildtype: "schsm", schsm_serial: "${env.PHYSHSM}", schsm_pin: "12345678")
			}
		} // stage 'Token Tests'
